...
class CategoryHook
{
   ...
   */
    public function hook_query_index(&$query,$request, $slug=NULL) {
        if( \Route::currentRouteName() == 'business-category.ajax-listing' ){
            $query->where('type','business');
        } else {
            $query->where('type','promote');
        }
    }
    ...
    */
    public function hook_before_add($request,&$postdata)
    {
        ...
        if( \Route::currentRouteName() == 'business-category.store'  ){
            $postdata['type'] = 'business';
        } else {
            $postdata['type'] = 'promote';
        }
